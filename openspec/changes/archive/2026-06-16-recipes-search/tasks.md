...
- [x] 3.1 Обновить `lib/bff/recipes.ts`: `getRecipes` принимает `q` и `sort`, пробрасывает в upstream URL
- [x] 3.2 Обновить `app/api/cookbook/v1/recipes/route.ts`: проброс параметров `q` и `sort` в gateway
- [x] 3.3 Добавить unit-тесты BFF: `recipes.bff.test.ts` — проверить что `q` и `sort` попадают в URL запроса
...
- [x] 4.1 Добавить поисковый инпут на страницу `app/(public)/page.tsx`; значение читается из `searchParams.q`; стиль по макету (`.search-wrap` + `.search-input` + иконка поиска)
- [x] 4.2 Добавить переключатель сортировки (`title_asc` / `title_desc`) в сайдбар; значение читается из `searchParams.sort`; стиль — `.aside-item` с `.is-active` для активного варианта
- [x] 4.3 При изменении поиска или сортировки обновлять URL (router.push с новыми searchParams), сбрасывать `page=1`
- [x] 4.4 Реализовать автодополнение: при вводе ≥ 2 символов последнего слова показывать подсказки из загруженных категорий и ингредиентов; стиль — `.autocomplete` + `.autocomplete-item` + `.kind` (см. макет)
- [x] 4.5 При выборе подсказки подставлять слово в поисковую строку
- [x] 4.6 Добавить story в Storybook для поискового инпута с автодополнением (`docs/design/storybook/src/stories/`)

## 5. Frontend — тесты

- [x] 5.1 Добавить UI e2e-тест в `tests/ui/test_recipes.py`: ввод поискового запроса фильтрует список рецептов
- [x] 5.2 Добавить UI e2e-тест: переключение сортировки меняет порядок карточек
